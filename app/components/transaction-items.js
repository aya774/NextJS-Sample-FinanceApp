...
export default function TransactionItem({
  ...
  const colors = typesMap[type]?.colors || 'text-gray-500';
  const formattedAmount = useFormat(amount);

  return (
    <div className="w-full flex items-center">
      <div className="flex items-center mr-4 grow">
        {IconComponent && <IconComponent className={`${colors} mr-2 w-4 h-4 hidden sm:block`} />}
      ...
      <div className="min-w-[100px] flex justify-end">
        <Link href={`/dashboard/transaction/${id}/edit`} className="ghost-xs">
